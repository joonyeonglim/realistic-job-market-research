...
The machine-readable SSOT is `assets/source-registry.json`: 13 implemented adapters, 15 access probes, and one authenticated handoff. A probe is never described as collected. Counts are derived per run. Source authorization and rate/retention policy live separately in `assets/source-governance.json`.
