...
Create the deterministic queue from exact raw identities before deep review:

```bash
node scripts/create-review-queue.mjs --run-dir /absolute/run --query "AI Agent" --limit 50
```

The queue is not a score input. Every employer and JD axis remains `UNRESOLVED` until evidence is attached; unselected or unresolved rows are never exclusions.
