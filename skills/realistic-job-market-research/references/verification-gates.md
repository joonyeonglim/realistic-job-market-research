...
14. **Reproducibility:** `record-qa.mjs` reruns the build from frozen raw inputs and hashes command output, manifest,
    identity list, desktop/mobile screenshots, and environment. `audit-run.mjs` verifies those hashes before PASS.
...
5. Links accept only HTTP(S), untrusted text is escaped, one `h1` and a skip link exist, controls have labels, and keyboard focus is visible.
