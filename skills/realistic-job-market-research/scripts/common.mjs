...
export function validateLedger(payload, options = {}) {
  ...
  if (!['live', 'imported', 'synthetic'].includes(metadata.snapshot_kind)) throw new Error(`${label}: snapshot_kind must be live, imported, or synthetic`);
  ...
  for (const [index, input] of metadata.inputs.entries()) {
    if (!input || !["live_http", "public_search", "main_chrome", "imported_snapshot", "named_targets", "synthetic_fixture"].includes(input.kind) || !isISODate(input.captured_at)) throw new Error(`${label}: metadata.inputs[${index}] is invalid`);
