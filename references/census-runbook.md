...
`expected_inputs`, `access_mode`, `scope`, `queries`, `pagination`, `output_path`, and `attempt_status`.
...
If review is in scope, `<run>/profile.json` records `profile_version`, `captured_at`, canonical profile source
...
```bash
node scripts/run-census.mjs --run-dir /absolute/path/to/YYYY-MM-DD-scope-census
```
