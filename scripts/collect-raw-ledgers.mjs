...
const stableSources = ["wanted", "saramin", "jumpit", "rallit"];
...
async function fetchText(url, options = {}) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      ...
      const requestedDelay = Number.isFinite(error.retryAfter) && error.retryAfter >= 0 ? error.retryAfter * 1_000 : 250 * (2 ** attempt);
      await sleep(Math.min(requestedDelay, 30_000));
    }
  }
  throw failure;
}
...
const textOnly = value => decodeHTML(String(value ?? "").replace(/<[^>]+>/g, " ")).replace(/\s+/g, " ").trim();

function parseSaraminCards(html, query) {
...
async function collectSaramin() {
  ...
  for (const query of queries) {
    ...
    for (const [pageIndex, payload] of payloads.entries()) {
      const parsed = parseSaraminCards(payload.innerHTML || "", query);
