    ...
    if isinstance(value, dict):
        for key, item in value.items():
            if key in FORBIDDEN_KEYS:
                errors.append(f"{path}.{key}: recruiter contact fields are forbidden")
            errors.extend(collect_forbidden(item, f"{path}.{key}"))
    elif isinstance(value, list):
        for index, item in enumerate(value):
            errors.extend(collect_forbidden(item, f"{path}[{index}]"))
    return errors
...
def validate_role(role: object, index: int) -> list[str]:
    ...
    validate_urls(
        require_list(role, "evidence", path, errors), f"{path}.evidence", errors
    )
